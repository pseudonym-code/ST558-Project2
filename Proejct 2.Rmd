...
```{r}
# Model 3: Linear model with 4 predictor variables because we drop hum from Model 1
f_m2<-as.formula("casual~season+workingday+weathersit+atemp")

fit_m3 <- train(f_m2, data = p2Train,
  method = "lm",
  preProcess = c("center","scale"),
  trControl = trainControl(method = "cv" , number = 10))
